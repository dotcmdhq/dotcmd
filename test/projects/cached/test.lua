...
test('cached downloads and prepares an archive only once', function()
    local calls = 0
    ...
    local options = { url = url .. '/archive', sha256 = sha256(archive), name = 'fresh.zip', prepare = prepare }
    local path = cached(options)
    assert(t.read(path .. '/sdk/lib/value') == 'library')
    options.url = url .. '/status/500'
    assert(cached(options) == path and calls == 1)
    local download = cached { url = options.url, sha256 = options.sha256, name = options.name }
...
test('cached preserves filenames and ignores URL query and fragment', function()
    ...
    assert(cached { url = 'https://127.0.0.1/', sha256 = hash, name = 'tool ü.exe' } == path)
...
test('cached preparation supports file outputs and ignores return values', function()
    ...
    assert(cached(options) == output and calls == 1)
    assert(cached { url = options.url, name = options.name, sha256 = hash } == input)
    assert(t.read(input) == 'original bytes')
end)

test('cached preparation is invalidated by callback bytecode', function()
    ...
    local stripped = cached { url = 'https://127.0.0.1/sdk.zip', sha256 = hash,
        prepare = assert(load(source:format(1)))() }
    local unstripped = cached { url = 'https://127.0.0.1/sdk.zip', sha256 = hash,
...
test('cached preparation is invalidated by download contents and filename', function()
    ...
    local function run(hash, name)
        return cached { url = 'https://127.0.0.1/' .. name, sha256 = hash, prepare = prepare }
    end
...
test('cached preparation deliberately does not fingerprint captured values', function()
    ...
    local first = cached(options)
    options.prepare = make_prepare('two')
    assert(cached(options) == first and t.read(first) == 'one')
end)

test('cached prepared results are shared across processes with identical stripped bytecode', function()
    ...
    local output = cached { url = 'https://127.0.0.1/shared-prep.txt', sha256 = hash,
    ...
    print(cached {url='https://127.0.0.1/shared-prep.txt', sha256=%q,
...
test('cached failed extraction publishes nothing and cleans temporary files', function()
    ...
    for _ = 1, 2 do
        ...
        function() cached { url = 'https://127.0.0.1/bad.zip', sha256 = hash, prepare = unpack_archive } end)
        assert_entries_unchanged(before)
    end
end)

test('cached failed preparation cleans partial files and directories and can retry', function()
    for _, directory in ipairs { false, true } do
        ...
        t.assert_error('prep failed', function() cached(options) end)
        assert_entries_unchanged(before)
        assert(fs.stat(input).type == 'file')
        fail = false
        local result = cached(options)
        assert(t.read(directory and (result .. '/value') or result) == 'finished')
        assert(cached(options) == result and calls == 2)
    end
end)

test('cached rejects missing outputs and invalid prepare callbacks', function()
    ...
    local before = prepared_entries()
    t.assert_error('prepare must create the output file or directory', function() cached(options) end)
    assert_entries_unchanged(before)
    for _, prepare in ipairs { false, 'script.lua', {}, extract } do
        options.prepare = prepare
        t.assert_error('prepare must be a Lua function', function() cached(options) end)
    end
end)

test('cached preserves a competing publication and discards the losing output', function()
