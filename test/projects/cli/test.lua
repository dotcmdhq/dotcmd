...
local function run(args, cwd) return t.run_project(child, args, cwd) end
...
test('CLI help without a project', function()
    for _, args in ipairs({{}, {'-h'}, {'--help'}}) do
        assert(success(t.run_project(empty, args)):find('--version', 1, true))
    end
end)
...
test('CLI clean errors for missing project and unknown command', function()
    ...
    assert(not failure(run({'unknown'}), 'unknown command'):find('stack traceback', 1, true))
end)
test('CLI definitions, descriptions, and Lua errors', function()
    assert(success(run({'--help'})):find('args  Print arguments as hex', 1, true))
    success(run({'nothing'}))
    failure(run({'invalid'}), 'integer exit code')
    failure(run({'crash'}), 'intentional project error')
