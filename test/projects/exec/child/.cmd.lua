return {
    ...
    large = function()
        ...
        for i = 1, 64 do io.write(string.rep('o', 65536)); io.stderr:write(string.rep('e', 65536)) end
    end,
    env = function(...)
        for i = 1, select('#', ...) do print(os.getenv(select(i, ...)) or '<missing>') end
    end,
    cwd = function() print(host.cwd) end,
    status = function(code) io.stderr:write('child failure'); return tonumber(code) end,
}
