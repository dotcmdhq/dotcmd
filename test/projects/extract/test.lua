...
test('extract accepts a positional destination with default options', function()
    assert(extract('fixtures/sdk.zip', 'positional ü'))
    assert(t.read('positional ü/sdk/lib/value') == 'library')
    assert(t.read('positional ü/sdk/share/ü.txt') == 'unicode')
    t.assert_error('destination already exists', function()
        extract('fixtures/sdk.zip', 'positional ü')
    end)
end)
