...
test('plugin downloads text source and returns all of its values', function()
    ...
    local hash = sha256(values_source)
...
test('plugin source names identify their URL in compile and runtime errors', function()
    ...
    t.assert_error(syntax_url, function() plugin(syntax_url, sha256(syntax_source)) end)
    local runtime_url = url .. '/plugin/runtime'
    t.assert_error(runtime_url, function() plugin(runtime_url, sha256(runtime_source)) end)
    t.assert_error('plugin runtime failure', function() plugin(runtime_url, sha256(runtime_source)) end)
end)
